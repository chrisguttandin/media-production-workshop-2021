...
module.exports = (grunt) => {
    return {
        'csp-production': {
            files: {
                'build/media-production-workshop-2021/browser/index.html': ['build/media-production-workshop-2021/browser/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: () => {
                            const html = fs.readFileSync('build/media-production-workshop-2021/browser/index.html', 'utf8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': ['build/media-production-workshop-2021/browser/ngsw.json']
            },
            options: {
                patterns: [
                    {
                    ...
                    },
                    {
                        ...
                        replacement: (_, filename) => {
                            return `"/media-production-workshop-2021/browser/${filename}.html": "${computeHashOfFile(
                                `build/media-production-workshop-2021/browser/${filename}.html`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
        ...
        'runtime': {
            files: {
                './': ['build/media-production-workshop-2021/browser/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, moduleAttribute) => {
                            if (moduleAttribute === undefined) {
                                return `<script>${fs.readFileSync(`build/media-production-workshop-2021/browser/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                            }

                            // eslint-disable-next-line node/no-sync
                            return `<script${moduleAttribute}>${fs.readFileSync(
                                `build/media-production-workshop-2021/browser/${filename}`
                            )}</script>`;
                        }
                    }
                ]

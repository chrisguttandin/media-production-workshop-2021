...
const replaceHashInMatch = (grunt, match, prefix, index) => {
    const filename = grunt.file.expand({ cwd: 'build/media-production-workshop-2021/scripts' }, `${prefix}.*.js`)[0];

    if (filename === undefined) {
        return match;
    }

    const hash = `sha384-${computeHashOfFile(`build/media-production-workshop-2021/scripts/${filename}`, 'sha384', 'base64')}`;
...
module.exports = (grunt) => {
    return {
        ...
        'chunks': {
            files: {
                './': ['build/media-production-workshop-2021/index.html']
            },
            options: {
                patterns: [
                    {
                ...
                patterns: [
                    {
                        ...
                        replacement: () => {
                            const html = fs.readFileSync('build/media-production-workshop-2021/index.html', 'utf8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': ['build/media-production-workshop-2021/ngsw.json']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, extension) =>
                            grunt.file.expand({ cwd: 'build/media-production-workshop-2021', ext: extension }, `assets/${filename}.*`)[0]
                    },
                    {
                    ...
                    {
                        match: /\s*"\/media-production-workshop-2021(?:\/scripts)?\/runtime(?:-es(?:2015|5))?.[\da-z]*\.js",/g,
                        replacement: ''
                    },
                    {
                        match: /\s*"\/media-production-workshop-2021(?:\/scripts)?\/runtime(?:-es(?:2015|5))?.[\da-z]*\.js":\s*"[\da-z]+",/g,
                        replacement: ''
                    },
                    {
                        // Replace the hash value inside of the hashTable for "/scripts/*.js" because it may have been modified before.
                        match: /"\/media-production-workshop-2021(?<filename>\/scripts\/(?:\d+|main|scripts)(?:-es(?:2015|5))?.[\da-z]+.js)":\s*"[\da-z]+"/g,
                        replacement: (_, filename) => {
                            return `"/media-production-workshop-2021${filename}": "${computeHashOfFile(
                                `build/media-production-workshop-2021${filename}`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
                    },
                    {
                        ...
                        match: /"\/media-production-workshop-2021(?<filename>\/styles\/styles\.[\da-z]*\.css)":\s*"[\da-z]+"/g,
                        replacement: (_, filename) => {
                            return `"/media-production-workshop-2021${filename}": "${computeHashOfFile(
                                `build/media-production-workshop-2021${filename}`,
                                'sha1',
                    ...
                    },
                    {
                        ...
                        match: /"\/media-production-workshop-2021\/(?<filename>index|start)\.html":\s*"[\da-z]+"/g,
                        replacement: (_, filename) => {
                            return `"/media-production-workshop-2021/${filename}.html": "${computeHashOfFile(
                                `build/media-production-workshop-2021/${filename}.html`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
                ...
                patterns: [
                    {
                        ...
                        replacement: (_, filename, moduleAttribute) => {
                            if (moduleAttribute === undefined) {
                                return `<script>${fs.readFileSync(`build/media-production-workshop-2021/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                            }
                        ...
                        replacement: (_, filename, moduleAttribute, deferAttribute, initialHash) => {
                            const updatedHash = /main(?:-es(?:2015|5))?\.[\da-z]+\.js/.test(filename)
                                ? `sha384-${computeHashOfFile(
                                      `build/media-production-workshop-2021/scripts/${filename}`,
                                      'sha384',
                                      'base64'
                                  )}`
                                : initialHash;
                    ...
                    },
                    {
                        ...
                        replacement: (_, filename, media) => {
                            const hash = `sha384-${computeHashOfFile(
                                `build/media-production-workshop-2021/styles/${filename}`,
                                'sha384',
                                'base64'
                            )}`;
